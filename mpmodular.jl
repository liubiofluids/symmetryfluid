using Pkg: installed
using Distributed
@everywhere using Pkg, Blink, SharedArrays, Printf, Spinnaker, Serialization, GLMakie, ImageCore, StaticArrays, BenchmarkTools, Images, CUDA, Makie.GeometryBasics, Dates
...
@everywhere function bestguesstrackingstartfunction!(theimagearray, r)
    ...
    @fastmath @inbounds @simd for theindex in eachindex(r)
        r[theindex] = maxintensitylocation[theindex]
    end
end

@everywhere function thetrackerfunction!(theimagearray, myflags, r, clickedlocation, imageanalysisparameters)
    ...
    # #Peeking at the processed image
    # #img=Observable(UInt8.(myclippedbool))
    # img=Observable(myfilteredclippedview)
    # #println("Made my makie observable")
    # imgplot = image(img,axis = (aspect=DataAspect(),),figure = (figure_padding=0, size=(600,600)))
    ...
    while myflags[6] == 1
        ...
        if myflags[12] == 1
            @fastmath @inbounds @simd for theindex in eachindex(r)
                r[theindex] = clickedlocation[theindex]
            end
            #println("This is my received r")
            #println(r)
            myflags[12] = 0
        end
        @. clipsize = Int(floor([imageanalysisparameters[1], imageanalysisparameters[2]]))
        ...
        #clipboxcoordcorrector!(r,clipsize,clipbotbot,imagesize)
        clipclipper!(clipsize, myclippedview, theimagearray, clipbotbot)
        imfilter!(myfilteredclippedview, myclippedview, mykernel)
        #m=masscalc(myclippedview)
        thresholdarray!(myclippedbool, myfilteredclippedview, currentthreshold)

        # #Peeking at the processed data
        # #println("I'm about to show the bool array")
        # #img[]=UInt8.(myclippedbool)
        # img[]=myfilteredclippedview
        # sleep(0)
        ...
        #println("This is my r at the end of the tracking loop")
        #println(r)
        # if (time()-roundstarttime)>10
        #     println("I have a round frequency of "*string(roundcounter/10))
        #     roundcounter=0
        #     roundstarttime=time()
        # else
        #     roundcounter+=1
...
@everywhere function newframecenter!(centroids, r, clipbotbot)
    ...
    #println(mycentroiddistances2)
    @fastmath @inbounds @simd for centroidindex in 1:length(centroids)-1
        mycentroiddistances2[centroidindex] = centroids[centroidindex][1]^2 + centroids[centroidindex][2]^2
    end
    ####println(mycentroiddistances2)
    thesmallestindex = argmin(mycentroiddistances2)
    ####println(thesmallestindex)
    #println(centroids[thesmallestindex])
    #centroids[thesmallestindex][1]
    #centroids[thesmallestindex][2]
    r .= clipbotbot .+ Int.(floor.(MVector(centroids[thesmallestindex][1], centroids[thesmallestindex][2])))
end


@everywhere function clipclipper!(clipsize, myclippedview, theimagearray, clipbotbot)
    @fastmath @inbounds for j in 1:clipsize[2]
        @simd for i in 1:clipsize[1]
            myclippedview[i, j] = theimagearray[clipbotbot[1]+i-1, clipbotbot[2]+j-1]
        end
    end
end

#function masscalc(myclippedview)
#    mass=0
#    @fastmath @inbounds @simd for idx in eachindex(myclippedview)
#        mass+=myclippedview[idx]<128 ? 0 : 1
#    end
#    mass
#end
...
# @everywhere function raverage(myclippedview,clipsize)
#     ravg=MVector(0,0,0)
# 	@fastmath @inbounds for j in 1:clipsize[2]
#         @simd for i in 1:clipsize[1]
#             ravg[1]+=ifelse(myclippedview[i,j],i,0)
#             ravg[2]+=ifelse(myclippedview[i,j],j,0)
#             ravg[3]+=myclippedview[i,j]
#         end
#     end
#     MVector(ravg[1],ravg[2])/ravg[3]
# end

#@everywhere function mysummer(myarray)
#    sum=0
#    @fastmath @inbounds @simd for idx in eachindex(myarray)
#        sum+=myarray[idx]
#    end
#    sum
#end

#@everywhere function clipboxcoordcorrector!(r,clipsize,clipbotbot,imagesize)
#    @turbo for i in 1:2
#        clipbotbot[i]=r[i]-clipsize[i]/2-min(0,r[i]-clipsize[i]/2)+min(0,imagesize[i]-r[i]-clipsize[i]/2)
#    end
#end
...
@everywhere function thedisplayfunction(theimagearray, myflags, clickedlocation, r, imageanalysisparameters, dest)
    ...
    on(events(imgplot).mousebutton, priority=2) do event
        if event.button == Mouse.left && event.action == Mouse.press
            ...
            #println("This is my window size")
            #println(windowsize)
            #println("This is my mouseclick location")
            #println(mouseclicklocation)
            if (mouseclicklocation[1] >= 1) && (mouseclicklocation[1] <= windowsize[1]) && (mouseclicklocation[2] >= 1) && (mouseclicklocation[2] <= windowsize[2])#For some reason the clicks on a Makie window sometimes report coordinates outside the domain of the image. This check filters the bad events.
                clickedlocation[1] = clamp(Int(floor(2048 * ((mouseclicklocation[1] / windowsize[1])))), 1, 2048)
                clickedlocation[2] = clamp(Int(floor(2048 * ((mouseclicklocation[2] / windowsize[2])))), 1, 2048)
                myflags[12] = 1
            end
        end
    end
    ...
    mypolyaoivar = poly!(mypolyaoi, color=:transparent, strokecolor=:red, strokewidth=2)
    ...
    while myflags[3] == 1
        ...
        if myflags[6] == 1
            ...
            @. clipsize = Int(floor([imageanalysisparameters[1], imageanalysisparameters[2]]))
            @. clipbotbot = clamp(r - Int(floor(clipsize / 2)), 1, imagesize - clipsize + 1)
            polygonaoipoints = Point2f[(clipbotbot[1], clipbotbot[2]), ((clipbotbot[1] + clipsize[1]), clipbotbot[2]), ((clipbotbot[1] + clipsize[1]), (clipbotbot[2] + clipsize[2])), (clipbotbot[1], (clipbotbot[2] + clipsize[2]))]
            mypolyaoi = Polygon(polygonaoipoints)
            #println("This is my bounding box")
            #println(polygonaoipoints)
            mypolyaoivar = poly!(mypolyaoi, color=:transparent, strokecolor=:red, strokewidth=2, alpha=0.5)
            mytrackedpoint = Point2f[(r[1], r[2])]
            #println("This is my tracked point")
...
@everywhere function thefancypants!(t, thefancypantsarray, instructionarray, landmarksarray, thefancypantspos, calctype)
        ...
        if t > landmarksarray[1, myinstructionsetround+1]
            ...
            if instructionarray[3, myinstructionsetround] == 1#cart
                if instructionarray[4, myinstructionsetround] == 1#diff
                    if 1 in calctype
                        @simd for velindex in 1:3
                            thefancypantsarray[velindex] = instructionarray[velindex+4, myinstructionsetround] * therisingtimeexpdot
                        end
                        @simd for velindex in 4:6
                            thefancypantsarray[velindex] = 0
                        end
                    end
                    if 0 in calctype
                        @simd for posindex in axes(thefancypantspos, 1)
                            thefancypantspos[posindex] = (instructionarray[posindex+4, myinstructionsetround] + landmarksarray[posindex+1, myinstructionsetround]) * therisingtimeexp + (landmarksarray[posindex+1, myinstructionsetround]) * thefallingtimeexp
                        end
                    end
                elseif instructionarray[4, myinstructionsetround] == 2#point
                    if 1 in calctype
                        @simd for velindex in 1:3
                            thefancypantsarray[velindex] = instructionarray[velindex+4, myinstructionsetround] * therisingtimeexpdot + landmarksarray[velindex+1, myinstructionsetround] * thefallingtimeexpdot
                        end
                        @simd for velindex in 4:6
                            thefancypantsarray[velindex] = 0
                        end
                    ...
                    if 0 in calctype
                        @simd for posindex in axes(thefancypantspos, 1)
                            thefancypantspos[posindex] = instructionarray[posindex+4, myinstructionsetround] * therisingtimeexp + landmarksarray[posindex+1, myinstructionsetround] * thefallingtimeexp
                        end
                    end
                end
            elseif instructionarray[3, myinstructionsetround] == 2#cyl
                if instructionarray[4, myinstructionsetround] == 1#phase
                    ϕ = therisingtimeexp * instructionarray[7, myinstructionsetround] + thefallingtimeexp * instructionarray[6, myinstructionsetround]
                    ϕdot = therisingtimeexpdot * instructionarray[7, myinstructionsetround] + thefallingtimeexpdot * instructionarray[6, myinstructionsetround]
                    if instructionarray[5, myinstructionsetround] == 3#z
                        if 1 in calctype
                            thefancypantsarray[1] = -sin(ϕ) * ϕdot * instructionarray[8, myinstructionsetround]
                            thefancypantsarray[2] = cos(ϕ) * ϕdot * instructionarray[8, myinstructionsetround]
                            @simd for velindex in 3:6
                                thefancypantsarray[velindex] = 0
                            end
                        end
                        if 0 in calctype
                            thefancypantspos[1] = (cos(ϕ) - cos(instructionarray[6, myinstructionsetround])) * instructionarray[8, myinstructionsetround] + landmarksarray[2, myinstructionsetround]
                            thefancypantspos[2] = (sin(ϕ) - sin(instructionarray[6, myinstructionsetround])) * instructionarray[8, myinstructionsetround] + landmarksarray[3, myinstructionsetround]
                    ...
                    if instructionarray[5, myinstructionsetround] == 1#dim
                        if instructionarray[6, myinstructionsetround] == 3#z
                            if instructionarray[7, myinstructionsetround] == 1#x
                                if 1 in calctype
                                    thefancypantsarray[1] = instructionarray[8, myinstructionsetround] * sign(cos(instructionarray[9, myinstructionsetround])) * therisingtimeexpdot
                                    thefancypantsarray[2] = instructionarray[8, myinstructionsetround] * sign(cos(instructionarray[9, myinstructionsetround])) * tan(instructionarray[9, myinstructionsetround]) * therisingtimeexpdot
                                    @simd for velindex in 3:6
                                        thefancypantsarray[velindex] = 0
                                    end
                                end
                                if 0 in calctype
                                    thefancypantspos[1] = (instructionarray[8, myinstructionsetround] * sign(cos(instructionarray[9, myinstructionsetround])) + landmarksarray[2, myinstructionsetround]) * therisingtimeexp + landmarksarray[2, myinstructionsetround] * thefallingtimeexp
                                    thefancypantspos[2] = (instructionarray[8, myinstructionsetround] * sign(cos(instructionarray[9, myinstructionsetround])) * tan(instructionarray[9, myinstructionsetround]) + landmarksarray[3, myinstructionsetround]) * therisingtimeexp + landmarksarray[3, myinstructionsetround] * thefallingtimeexp
                                    thefancypantspos[3] = landmarksarray[4, myinstructionsetround]
                                end
                            end
                        end
...
@everywhere function thefancypantsport!(t, thefancypantsportarray, instructionportarray, landmarksportarray, thefancypantsportconfig, calctype)
                ...
                if instructionportarray[4, myinstructionsetround] == 2#point
                    if 0 in calctype
                        @simd for posindex in axes(thefancypantsportconfig, 1)
                            thefancypantsportconfig[posindex] = instructionportarray[posindex+4, myinstructionsetround]
                        end
                    end
                end
            end
...
@everywhere function fancypantslandmarks!(instructionarray, landmarksarray, customcrunchmetadata)
    t₀ = landmarksarray[1, 1]
    t₁ = landmarksarray[1, 1]
    thefancypantspos = MVector{3,Float64}(0, 0, 0)
    thefancypantsarray = MVector{6,Float64}(0, 0, 0, 0, 0, 0)
    @fastmath @inbounds for myinstructionsetround in 1:customcrunchmetadata[1]
        t₀ = t₁
        t₁ = t₁ + instructionarray[1, myinstructionsetround]
        landmarksarray[1, myinstructionsetround+1] = t₁
        thefancypants!(t₁, thefancypantsarray, instructionarray, landmarksarray, thefancypantspos, [0])
        @simd for posindex in axes(thefancypantspos, 1)
...
@everywhere function fancypantslandmarksport!(instructionportarray, landmarksportarray, customportcrunchmetadata)
    ...
    @fastmath @inbounds for myinstructionsetround in 1:customportcrunchmetadata[1]
        t₀ = t₁
        t₁ = t₁ + instructionportarray[1, myinstructionsetround]
        landmarksportarray[1, myinstructionsetround+1] = t₁
        thefancypantsport!(t₁, thefancypantsportarray, instructionportarray, landmarksportarray, thefancypantsportconfig, [0])
        @simd for posindex in axes(thefancypantsportconfig, 1)
            landmarksportarray[posindex+1, myinstructionsetround] = thefancypantsportconfig[posindex]
        end
    end
end
...
@everywhere function thepressurecruncher(myflags, mypressurecruncherarray, mymodeamounts, mymodescalingamounts, myportassignments, combinedcrunchmodeamounts, landmarksarray, instructionarray, customcrunchmetadata, r, trackplanescale, custommodescale, customportscale, instructionportarray, landmarksportarray, customportcrunchmetadata,trackcoords,dest)
    ...
    while myflags[5] == 1
        if myflags[7] == 1 && myflags[6] == 1
            ...
            @fastmath @inbounds @simd for trackedindex in 1:2
                thefancypantsarray[trackedindex] = thefancypantsarray[trackedindex] + trackplanescale[trackedindex] * (thefancypantspos[trackedindex] - r[trackcoords[trackedindex]]) / avgcrunchtime
            end
        elseif myflags[7] == 1
            thefancypants!((time() - starttime) % landmarksarray[1, customcrunchmetadata[1]+1], thefancypantsarray, instructionarray, landmarksarray, thefancypantspos, [1])
        else
            thefancypantsarray .= 0
        end
        if myflags[11] == 1
            thefancypantsport!((time() - starttime) % landmarksportarray[1, customportcrunchmetadata[1]+1], thefancypantsportarray, instructionportarray, landmarksportarray, thefancypantsportconfig, [0])
        else
            thefancypantsportconfig .= 0
        end
        @fastmath @inbounds @simd for i in eachindex(combinedcrunchmodeamounts)
            combinedcrunchmodeamounts[i] = mymodeamounts[i] + custommodescale[1] * thefancypantsarray[i]
        end
        @fastmath @inbounds @simd for i in eachindex(mypressurecruncherarray)
            mypressurecruncherarray[i] = combinedcrunchmodeamounts[1] * mymodescalingamounts[1] * xflow[myportassignments[i]] + combinedcrunchmodeamounts[2] * mymodescalingamounts[2] * yflow[myportassignments[i]] + combinedcrunchmodeamounts[3] * mymodescalingamounts[3] * zflow[myportassignments[i]] + combinedcrunchmodeamounts[4] * biasflow[myportassignments[i]] + combinedcrunchmodeamounts[5] * opxflow[myportassignments[i]] + combinedcrunchmodeamounts[6] * opyflow[myportassignments[i]] + customportscale[1] * thefancypantsportconfig[myportassignments[i]]
        end
        # if (time()-roundstarttime)>10
        #     println("I have a round frequency of "*string(roundcounter/10))
        #     roundcounter=0
...
@everywhere function theelveflowfunction(myflags, myreadpressure, mypressurecruncherarray, myoffsetpressure, maxabsp, myportscaling, thetopleveldatadir, recordfoldernumber)
...
@everywhere function thepointgreycamerafunction(theimagearray, myflags, thetopleveldatadir, recordfoldernumber)
    ...
    while myflags[2] == 1
        ...
        if myflags[4] == 1
            ...
            savebyserial(thetopleveldatadir * lpad(recordfoldernumber[1], 5, "0") * "/cam0/" * @sprintf("%.9f", time()) * "_" * string(imid) * "_" * string(imtimestamp) * ".slz", imagefromcamera) #Apparently serializing is fast, but it needs an actual array to work with, otherwise serializing will just store what appears to be a pointer to the data in memory, and once the processes have ended then all that goes away.
        end
    end
...
@everywhere function crunchcustomparser!(instructionstring, instructionarray, landmarksarray, customcrunchmetadata)
        ...
        if thelinenumber == 1
            ...
            if theinstructionelements[3] == "cart"
                ...
                if theinstructionelements[4] == "diff"
                    ...
                    for item in 5:7
                        instructionarray[item, thelinenumber-1] = eval(Meta.parse(theinstructionelements[item]))
                    end
...
@everywhere function crunchcustomportparser!(instructionstring, instructionportarray, landmarksportarray, customportcrunchmetadata)
    allthelines = split(instructionstring, "\n")
    for (thelinenumber, theline) in enumerate(allthelines)
        theinstructionelements = split(theline, ";")
        if thelinenumber == 1
            for (theelementnumber, theelement) in enumerate(theinstructionelements)
                landmarksportarray[theelementnumber, thelinenumber] = eval(Meta.parse(theelement))
            end
        else
            customportcrunchmetadata[1] = thelinenumber - 1
            instructionportarray[1, thelinenumber-1] = eval(Meta.parse(theinstructionelements[1]))
            ...
            if theinstructionelements[3] == "cart"
                ...
                if theinstructionelements[4] == "point"
                    ...
                    for item in 5:12
                        instructionportarray[item, thelinenumber-1] = eval(Meta.parse(theinstructionelements[item]))
                    end
                end
            end
        end
...
@everywhere function thestagefunction!(myflags, myreadstage, mysetstage)
    ...
    while myflags[8] == 1
        ...
        if myflags[9] == 1
            ret = PriorScientificSDK_cmd(sessionID, "controller.stage.goto-position $(mysetstage[1]) $(mysetstage[2])", rx)
            sleep(0.1)
        end
    end
...
@everywhere function squishanddisplay(theimagearray, myflags, squishparameters)
    ...
    while myflags[13] == 1
        ...
        # println("Intermediate image")
        # println(camholder[1:5,1:5])
        copyto!(gpuholder, camholder)
        gpuarraystack[framenumber, :, :] .= gpuholder
        # println("Copied slice")
        # println(Array(gpuarraystack[framenumber,1:5,1:5]))
        # println("Maximum slice")
        # println(Array(maximum(gpuarraystack,dims=1))[1,1:5,1:5])
        ...
        # println("Copied maximum slice")
        # println(mysquishedframe[1:5,1:5])
        img[] = mysquishedframe
...
function therunner()
    ...
    thetopleveldatadir = raw"C:/Users/Jeremias/Desktop/recordeddata" * Dates.format(Dates.now(), "yyyymmddHHMMSS") * "/"
    ...
    while active(w)
        ...
        myportscaling[1] = js(w, Blink.JSString("""document.getElementById("lpb1ssl").valueAsNumber"""))
        myportscaling[2] = js(w, Blink.JSString("""document.getElementById("lpb2ssl").valueAsNumber"""))
        myportscaling[3] = js(w, Blink.JSString("""document.getElementById("lpb3ssl").valueAsNumber"""))
        myportscaling[4] = js(w, Blink.JSString("""document.getElementById("lpb4ssl").valueAsNumber"""))
        myportscaling[5] = js(w, Blink.JSString("""document.getElementById("rpb1ssl").valueAsNumber"""))
        myportscaling[6] = js(w, Blink.JSString("""document.getElementById("rpb2ssl").valueAsNumber"""))
        myportscaling[7] = js(w, Blink.JSString("""document.getElementById("rpb3ssl").valueAsNumber"""))
        myportscaling[8] = js(w, Blink.JSString("""document.getElementById("rpb4ssl").valueAsNumber"""))
        ...
        mymodescalingamounts[1] = js(w, Blink.JSString("""document.getElementById("xmodescalesl").valueAsNumber"""))
        mymodescalingamounts[2] = js(w, Blink.JSString("""document.getElementById("ymodescalesl").valueAsNumber"""))
        mymodescalingamounts[3] = js(w, Blink.JSString("""document.getElementById("zmodescalesl").valueAsNumber"""))
        ...
        mycurrentcrunchcustomstate = js(w, Blink.JSString("""document.getElementById("crunchcustomon").checked"""))
        mycurrentcrunchcustomportstate = js(w, Blink.JSString("""document.getElementById("crunchcustomporton").checked"""))
        ...
        mycurrenttrackingstate = js(w, Blink.JSString("""document.getElementById("trackon").checked"""))
        mycurrenttextcustom = js(w, Blink.JSString("""document.getElementById("mytextcustom").value"""))
        mycurrenttextcustomport = js(w, Blink.JSString("""document.getElementById("mytextcustomport").value"""))
        mycurrentstagestate = js(w, Blink.JSString("""document.getElementById("stageon").checked"""))
        mycurrentstageupdatestate = js(w, Blink.JSString("""document.getElementById("stageupdateon").checked"""))
        mycurrenttrackbestguessstate = js(w, Blink.JSString("""document.getElementById("trackbestguesson").checked"""))
        mycurrentsquishdispstate = js(w, Blink.JSString("""document.getElementById("squishdisplayon").checked"""))
        mycurrentexperimentalnotes = js(w, Blink.JSString("""document.getElementById("experimentalnotes").value"""))

        custommodescale[1] = js(w, Blink.JSString("""document.getElementById("custommodescalesl").valueAsNumber"""))
        customportscale[1] = js(w, Blink.JSString("""document.getElementById("customportscalesl").valueAsNumber"""))
        ...
        js(w, Blink.JSString("""document.getElementById("sxr").innerHTML="$(stagereadstringholder[1])" """))
        js(w, Blink.JSString("""document.getElementById("syr").innerHTML="$(stagereadstringholder[2])" """))

        trackplanescale[1] = js(w, Blink.JSString("""document.getElementById("trackplanescalexsl").valueAsNumber"""))
        trackplanescale[2] = js(w, Blink.JSString("""document.getElementById("trackplanescaleysl").valueAsNumber"""))
        imageanalysisparameters[1] = js(w, Blink.JSString("""document.getElementById("clipsizeisl").valueAsNumber"""))
        imageanalysisparameters[2] = js(w, Blink.JSString("""document.getElementById("clipsizejsl").valueAsNumber"""))
        imageanalysisparameters[3] = js(w, Blink.JSString("""document.getElementById("imageanalysisthresholdsl").valueAsNumber"""))
        imageanalysisparameters[4] = js(w, Blink.JSString("""document.getElementById("kernelparametersl").valueAsNumber"""))
        mycurrenttrackcoordstate = js(w, Blink.JSString("""document.getElementById("trackxy").checked"""))

        squishparameters[1] = js(w, Blink.JSString("""document.getElementById("squishstacksizesl").valueAsNumber"""))
        squishparameters[2] = js(w, Blink.JSString("""document.getElementById("squishdelaysl").valueAsNumber"""))

        if (mycurrentpumpstate) && (oldpumpstate == 0)
            println("I'm about to start the new pump process")
            myflags[1] = 1
            push!(pumpfutures, @spawnat :any theelveflowfunction(myflags, myreadpressure, mypressurecruncherarray, myoffsetpressure, maxabsp, myportscaling, thetopleveldatadir, recordfoldernumber))
        ...
        if (mycurrentcrunchcustomstate) && (oldcrunchcustomstate == 0)
            ...
            crunchcustomparser!(mycurrenttextcustom, instructionarray, landmarksarray, customcrunchmetadata)
            ...
            @fastmath @inbounds for timeindex ∈ timerange
                thefancypants!(mytimepoints[timeindex] % landmarksarray[1, customcrunchmetadata[1]+1], thefancypantsarray, instructionarray, landmarksarray, thefancypantspos, [0])
                @simd for thepos ∈ 1:3
                    mypositionpoints[thepos, timeindex] = thefancypantspos[thepos]
                end
            end
            #pathgenfig=Figure(size=(300,300))
            pathgenplot = scatter(mypositionpoints[1, :], mypositionpoints[2, :], mypositionpoints[3, :], color=timerange, colormap=:rainbow)
            display(pathgenplot)
            myflags[7] = 1
        ...
        if (mycurrentcrunchcustomportstate) && (oldcrunchcustomportstate == 0)
            ...
            crunchcustomportparser!(mycurrenttextcustomport, instructionportarray, landmarksportarray, customportcrunchmetadata)
            fancypantslandmarksport!(instructionportarray, landmarksportarray, customportcrunchmetadata)
            myflags[11] = 1
        ...
        if (mycurrenttrackingstate) && (oldtrackerstate == 0)
            ...
            myflags[6] = 1
            push!(trackerfutures, @spawnat :any thetrackerfunction!(theimagearray, myflags, r, clickedlocation, imageanalysisparameters))
            oldtrackerstate = 1
        ...
        if (mycurrentstagestate) && (oldstagestate == 0)
            ...
            myflags[8] = 1
            push!(stagefutures, @spawnat :any thestagefunction!(myflags, myreadstage, mysetstage))
        ...
        if mycurrentstageupdatestate
            mysetstage[1] = parse(Int, js(w, Blink.JSString("""document.getElementById("sxs").value""")))
            mysetstage[2] = parse(Int, js(w, Blink.JSString("""document.getElementById("sys").value""")))
            myflags[9] = 1
        else
            myflags[9] = 0
        end
        ...
        if (mycurrentsquishdispstate) && (oldsquishdispstate == 0)
            ...
            myflags[13] = 1
            push!(squishdispfutures, @spawnat :any squishanddisplay(theimagearray, myflags, squishparameters))
